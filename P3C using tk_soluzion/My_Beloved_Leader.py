...
def playbgm():
    global ppp
    num=choice(list(range(1)))
    filename=str(num+1) +".wav"
    if sys.platform == 'darwin':
        ppp=subprocess.call(["afplay", filename])
    else:
        ppp=winsound.PlaySound(filename, winsound.SND_NODEFAULT)
    playbgm()
...
def stop_thread(thread):
    global ppp
    if sys.platform == 'darwin':
        subprocess.Popen.terminate(ppp)
    else:
        winsound.PlaySound(ppp, winsound.SND_PURGE)
        print(ppp+"has been purged")
    _async_raise(thread.ident, SystemExit)


t=threading.Thread(target=playbgm)
t.start()
...
class State:
    ...
    def make(self, choice_policy):
        ...
        if (choice_policy != "money forward" and choice_policy != "money backward"):
            ...
            if news.gov_support < 50:
                print("You cannot make this policy! You have only " + str(news.gov_support) + "% support rate.")
            else:
                # left wing parties
                if (choice_faction == "socialist" or choice_faction == "liberal"):
                    if (choice_policy == "immigration forward"):
        ...
        if (choice_policy == "money forward" or choice_policy == "money backward"):
            money = 0
            choice = input("Where do you want to spend/save money on" \
            ...
            while ask_money:
                if (choice == 'a'):
                    money = int(input("How much money would you like to spend/save (up to 15$): "))
                    key = "invest in employment centers"
                elif (choice == 'b'):
                    money = int(input("How much money would you like to spend/save (up to 10$): "))
                    key = "invest in public transportation"
                elif (choice == 'c'):
                    money = int(input("How much money would you like to spend/save (up to 5$): "))
        ...
        elif (news.gov_support >= 50):
            subprocess.call(["afplay", "whistle-slide-down.wav"])
    ...
    def make_double(self, choice_policy):
        ...
        if self.citizen_approval >78:
            self.citizen_approval -= uniform(6, 10)
    ...
    def increase_support(self, choice_policy):
        ch1 = int(input("How much pp do you want to convert?"))
...
def goal_message(s):
    return "You have successfully become the beloved leader of " + s.choice_con + "!"
...
# let the player choose a role
'''con_bool = False
choice_continent = ''
choice_faction = ''
while not con_bool:
    choice_continent = input(
        "Please choose a continent that deals with an immigration problem(enter in abbreviation: NA/EU/OC): ")
    if choice_continent != 'NA' and choice_continent != 'EU' and choice_continent != 'OC':
        print("Invalid input!")
        print("Please choose continent again: ")
    else:
        con_bool = True

fac_bool = False
while not fac_bool:
    choice_faction = input("Please choose a faction(socialist/liberal/conservative/populist/centralist): ")
    if choice_faction != 'socialist' and choice_faction != 'liberal' and choice_faction != 'conservative' \
            and choice_faction != 'populist' and choice_faction != 'centralist':
        print("Invalid input!")
        print("Please choose faction again: ")
    else:
        fac_bool = True
'''
